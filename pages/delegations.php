<div class="wrapper delegations">

	<div class="container">

		<!-- Content Section Begins -->

		<div id="content-delegations" class="justify-center">
			
			<h2>Delegations 2019</h2>
			
			<div class="row">
				
				<div class="col-md-4">
					<ul class="list justify-left">
					<li>2KnowHow2</li>
					<li>A.C.B.L Conseils</li>
					<li>A2jtech.com</li>
					<li>Aalto University</li>
					<li>ABN AMRO Bank N.V.</li>
					<li>Adarve Abogados</li>
					<li>Addleshaw Goddard</li>
					<li>Agens Oy</li>
					<li>Airbus</li>
					<li>Airbus Defence and Space GmbH</li>
					<li>Airbus ExO Alpha SAS</li>
					<li>Allen & Overy</li>
					<li>ALM Partners</li>
					<li>Alma Talent</li>
					<li>Amer Sports Oyj</li>
					<li>American Express</li>
					<li>Amiqus</li>
					<li>Amurabi</li>
					<li>Andrade Miranda Advogados</li>
					<li>Anna Langier Ltd</li>
					<li>Arriello, Regulatory Affairs and Pharmacovigilance</li>
					<li>Ascon</li>
					<li>Asianajotoimisto Salo & Salo Oy</li>
					<li>Asianajotoimisto Vision Oy</li>
					<li>ASML</li>
					<li>Assently AB</li>
					<li>Astrid Kohlmeier</li>
					<li>Atos</li>
					<li>Avarn</li>
					<li>Avokaado.com OÜ</li>
					<li>Axis Legal Ghana</li>
					<li>Basware Oyj</li>
					<li>BIOCAD</li>
					<li>Blueprint Genetics</li>
					<li>Bonfide Oy</li>
					<li>Bot&Partners</li>
					<li>Business College Helsinki</li>
					<li>CAPSTAN LMS</li>
					<li>Cassius Avocats</li>
					<li>Castén & Snellman</li>
					<li>CatalystZone</li>
					<li>City of Helsinki</li>
					<li>CMS</li>
					<li>Colombian Peace Tribunal </li>
					<li>Colt</li>
					<li>Contract Mill Oy</li>
					<li>Corporate Accountability Lab</li>
					<li>Damian Curran</li>
					<li>Danske Bank</li>
					<li>Deloitte</li>
					<li>Demos Helsinki</li>
					<li>Design Forum Finland</li>
					<li>District Court of Helsingborg</li>
					<li>Dittmar & Indrenius Attorneys Ltd</li>
					<li>DLA Piper UK LLP</li>
					<li>DNA Oyj</li>
					<li>Domnern Somgiat & Boonma Law Office Ltd.</li>
					<li>Dottir Attorneys</li>
					<li>DRA Consulting Oy</li>
					<li>Draugiem Group</li>
					<li>DWF Ventures</li>
					<li>Edita Publishing Oy</li>
					<li>Eickhoff Kalaoja</li>
					<li>Eliott & Markus</li>
					<li>Elisa Oyj</li>
					<li>Energy Authority of Finland / Energiavirasto</li>
					<li>ERAGE</li>
					<li>Ethel</li>
					<li>Etic Lab</li>
					<li>European Centre on Data Privacy and Cybersecurity</li>
					<li>European Chemicals Agency</li>
					<li>European Legal Tech Association - ELTA</li>
					<li>EY</li>
					<li>Finnish Commerce Federation / Kaupan liitto ry</li>
					<li>Finnish Education Employers /  Sivista</li>
					<li>Finnish Government</li>
					<li>Finnish Immigration Service</li>
					<li>Finnish Legal Tech Forumm</li>
					<li>Finnish Patent and Registration Office: PRH </li>
					<li>Finnish Procurement Lawyers Attorneys Ltd.</li>
					<li>Finnish Tax Administration</li>
					<li>Finnish Transport and Communications Agency TRAFICOM</li>
					<li>FKA Furtek Komosa Aleksandrowicz</li>
					<li>Fondia Oyj</li>
					<li>Frank Bold</li>
					<li>FrogSlayer</li>
					<li>Future in Law</li>
					<li>Futurice Oy</li>
					<li>Gofore</li>
					<li>Gowling WLG</li>
					</ul>
				</div>
				
				<div class="col-md-4">
					<ul class="list justify-left">
						<li>Graphic Advocacy Project</li>
						<li>Haaga Helia UAS</li>
						<li>Haglex Oy</li>
						<li>Hannes Snellman</li>
						<li>Háptica - Diseño de Servicios y Estrategias</li>
						<li>Harvard University</li>
						<li>Hellon</li>
						<li>Helsingborg District Court</li>
						<li>Helsinki Design Week</li>
						<li>HighQ</li>
						<li>Houthoff</li>
						<li>HPP Attorneys</li>
						<li>IGSG - CNR (Italian National Research Council)</li>
						<li>Ilves Solutions Oy</li>
						<li>iManage</li>
						<li>IMERYS</li>
						<li>Incorporated Council of Law Reporting for England and Wales</li>
						<li>InfoCert SpA</li>
						<li>Infotropic Media</li>
						<li>inland design</li>
						<li>InvestCEE LegalTech Consultancy</li>
						<li>JAM Advisors Oy</li>
						<li>Janders Dean</li>
						<li>JURO Limited</li>
						<li>JUST: Access Ltd</li>
						<li>Kanzlei Meising</li>
						<li>Kent Law School</li>
						<li>Kira Systems</li>
						<li>Kojamo Oyj</li>
						<li>Kopiosto</li>
						<li>KPMG</li>
						<li>KPMG Law Rechtsanwaltsgesellschaft mbH</li>
						<li>La Law Box</li>
						<li>Lakily Oy</li>
						<li>Lakipalvelu Kevytjuridiikka Oy</li>
						<li>Lancashire Law School UCLan</li>
						<li>LAPISARDI AVOCATS</li>
						<li>Lasker</li>
						<li>Laura Fauqueur</li>
						<li>Laurea University of Applied Sciense</li>
						<li>Law Society Gazette</li>
						<li>Lawgitech</li>
						<li>LCA Studio Legale</li>
						<li>Le Wagon</li>
						<li>Lean Entries Ltd</li>
						<li>LECARE GmbH</li>
						<li>Leegal Oy</li>
						<li>Lefebvre Sarrut</li>
						<li>Legal Army</li>
						<li>Legal Design Lab of the University of Andes</li>
						<li>Legal Design Lab, Stanford Law </li>
						<li>Legal Design Studio</li>
						<li>Legal Hackers</li>
						<li>Legal Tech Lab</li>
						<li>Legalcomplex</li>
						<li>Legalnext</li>
						<li>Legaltree</li>
						<li>LegalVision</li>
						<li>LexAnalitica</li>
						<li>Lexia Attorneys Ltd</li>
						<li>Lexpansio</li>
						<li>Lexpert Ltd </li>
						<li>Lijntje Zandee</li>
						<li>LINKVALUE</li>
						<li>LMU Munich</li>
						<li>Luhta Sportswear Company</li>
						<li>Lydeadesign</li>
						<li>Mars Inc.</li>
						<li>Matheson</li>
						<li>McKinsey</li>
						<li>Melbourne Law School</li>
						<li>Merilampi</li>
						<li>Microsoft</li>
						<li>Miller & Company</li>
						<li>Miltton</li>
						<li>Ministry of Justice</li>
						<li>MLTech</li>
						<li>Moscow State University</li>
						<li>Nathalie Bowman Legal</li>
						<li>National Police Board of Finland</li>
						<li>Nauta Dutilh</li>
						<li>Nextlaw Labs</li>
						<li>Nike Inc.</li>
						<li>Nixu Oyj</li>
						<li>Nordea Finance</li>
						<li>Nordia Law</li>
						<li>Nordx Legal</li>
						<li>Northeastern University School of Law</li>
						<li>Observ</li>
						<li>OlarteMoure</li>					
					</ul>
				</div>
				
				<div class="col-md-4">
					<ul class="list justify-left">
						<li>Open State Foundation</li>
						<li>OriginalMy Blockchain</li>
						<li>Ornamo Art and Design Finland</li>
						<li>Ozan&Ozan Law Office</li>
						<li>Palvelumuotoilu Palo</li>
						<li>Passera Design / IACCM</li>
						<li>Paul Hastings LLP</li>
						<li>PayU</li>
						<li>Pérez Bustamante & Ponce</li>
						<li>Philips</li>
						<li>PoLI Institute</li>
						<li>Postbank</li>
						<li>PricewaterhouseCoopers</li>
						<li>Primeo Oy</li>
						<li>PrivacyAnt</li>
						<li>Procopé & Hornborg Attorneys Ltd.</li>
						<li>Reaktor</li>
						<li>reThinkLegal GmbH</li>
						<li>Riscography solutions GmbH</li>
						<li>SAGAN Avocats</li>
						<li>Saint Petersburg State University</li>
						<li>Sarastia Oy</li>
						<li>Savvy</li>
						<li>Secrays</li>
						<li>Signatura</li>
						<li>Silo.AI</li>
						<li>Sis Ventures Ltd.</li>
						<li>Sitra</li>
						<li>Société québécoise d’information juridique</li>
						<li>Sorainen</li>
						<li>SPX FLOW</li>
						<li>SRC</li>
						<li>Stadin ammattiopisto</li>
						<li>Stanford University</li>
						<li>Startup Foundation</li>
						<li>Stibbe</li>
						<li>Stockholm university</li>
						<li>Studio Rossini</li>
						<li>Supreme Administrative Court of Finland</li>
						<li>Swisscom switzerland ltd</li>
						<li>Taaleri</li>
						<li>Taylor Wessing LLP</li>
						<li>Technology Industries of Finland</li>
						<li>Telia Company</li>
						<li>The Association of Finnish Lawyers</li>
						<li>The City of Helsinki</li>
						<li>The City of Kerava</li>
						<li>The Finnish Musicians’ Union</li>
						<li>The Legal Design Lab - Colombia</li>
						<li>The Technolawgist</li>
						<li>The Julius Maximilian University of Würzburg</li>
						<li>The University of Los Andes D.School</li>
						<li>This Is Legal Design</li>
						<li>Turre Legal</li>
						<li>Ubisoft</li>
						<li>UCLan</li>
						<li>UNDP</li>
						<li>Université catholique de Louvain</li>
						<li>University of Applied Sciences Leiden</li>
						<li>University of Arizona</li>
						<li>University of Bayreuth</li>
						<li>University of Eastern Finland</li>
						<li>University of Edinburgh </li>
						<li>University of Graz</li>
						<li>University of Helsinki</li>
						<li>University of Lancashire</li>
						<li>University of Lapland</li>
						<li>University of London</li>
						<li>University of Luxembourg</li>
						<li>University of Neaples Suor Orsola Benincasa</li>
						<li>University of Paris 1 Pantheon-Sorbonne</li>
						<li>University of Turku</li>
						<li>University of Vaasa</li>
						<li>University of Vienna</li>
						<li>UWE</li>
						<li>Vanderbilt Law School </li>
						<li>Waselius & Wist</li>
						<li>Wavelength.law</li>
						<li>Wiidare</li>
						<li>Wiidare Ltd</li>
						<li>WILDCAT</li>
						<li>Visual Contracts</li>
						<li>Vivo energy Tunisia</li>
						<li>Wunderdog</li>
						<li>Väestörekisterikeskus D9</li>
						<li>Yleisradio Oy</li>
						<li>Young AvoTech</li>
						<li>Åbo Akademi</li>
					</ul>
				</div>
				
			</div>
			
		</div>

		<!-- Content Main Section Ends -->

	</div>

</div>
